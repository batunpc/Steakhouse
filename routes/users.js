...
router.post(
      ...
      .custom((userEmail) => {
        return new Promise((resolve, reject) => {
          User.findOne({
            ...
            if (emailExist !== null) reject(new Error("Email already exists."));
            else resolve(true);
          });
        });
      }),
  ...
  (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      let isAdminToBool;
      if (is_admin == "admin") isAdminToBool = true;
      else if (is_admin == "client") isAdminToBool = false;
                ...
                .then(function () {
                  User.create({
                    ...
                    .then((userSaved) => {
                      if (isAdminToBool)
                        console.log(
                          `=> 🚀 Administrator: "${userSaved.username}" has registered`
                        );
                      else
                        console.log(
                          `=> 🚀 Customer: "${userSaved.username}" has registered`
                        );
      ...
      .custom((email) => {
        return new Promise((res, rej) => {
          User.findOne({
            ...
            .then((emailExist) => {
              if (emailExist !== null) res(true);
              else rej(new Error("email doesnt exist."));
            })
            .catch((err) => console.log(err));
        });
      }),
          ...
          if (userlogin) {
            bcrypt
              ...
              .then((match) => {
                if (match) {
                  console.log(
                    `User logged in with username: ${userlogin.email} 🚀`
                  );
