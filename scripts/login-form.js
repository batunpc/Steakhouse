...
formSignIn.addEventListener("submit", (event) => {
  ...
  validateLogin();
  if (validateEmail(email_signIn.value) && !(password_signIn.value === "")) {
    // only chechs if null exists in field
...
validateLogin = () => {
  ...
  //=====
  if (emailInput === "") errorMsg(email_signIn, "Email is required");
  else if (!validateEmail(emailInput))
    errorMsg(email_signIn, "Please enter a valid email");
  else successMsg(email_signIn);
  //=====
  if (passwordInput === "") errorMsg(password_signIn, "Password is required");
  else successMsg(password_signIn);
};
